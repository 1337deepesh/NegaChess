...
end_flag = 'NULL' #'W_WIN' 'B_WIN' 'STALEMATE'
...
def main():
    global clock, display_surface, square_size, database, master_board
    global move_flags, turn
    pygame.init()
    ...
    while True:
        ...
        for event in EVENT:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and downclick_flag == False:
                ...
                board_visualizer()
                legal_moves = piece_highlighter(position1)
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and downclick_flag == True:
                ...
                piece_mover(position1, position2, legal_moves)
                board_visualizer()
...
def checkmate_event(turn, move_flags, master_board):
    ...
    threat_map = threat_mapper(master_board)
    ...
    for i in range(0,8):
        for j in range(0,8):
            if turn in master_board[i][j]:
                counter_flag += len(king_defender([i,j], threat_map))
                if counter_flag != 0:
                    return
    ...
    for i in range(0,8):
        for j in range(0,8):
            if turn in master_board[i][j]:
                ...
                move_counter += len(move_lister(master_board, [i,j], threat_map))
...
def piece_mover(position1, position2, legal_moves):
    global clock, display_surface, square_size, database, master_board, piece_mapper
    global move_flags, turn
...
def move_lister(master_board, piece_coordinates, threat_map):
    global move_flags, turn
...
def threat_lister(master_board, piece_coordinates):
    global move_flags, turn
    color = master_board[piece_coordinates[0]][piece_coordinates[1]][0:1]
...
def piece_highlighter(position):
    global clock, display_surface, square_size, database, master_board, piece_mapper
    global move_flags, turn
    ...
    legal_moves = king_defender(piece_coordinates, threat_map)
...
def king_defender(piece_coordinates, threat_map):
    global clock, display_surface, square_size, database, master_board, piece_mapper
    global move_flags, turn
    
    current_piece = master_board[piece_coordinates[0]][piece_coordinates[1]]
    #calculate all normally legal moves for selected piece:
    legal_moves = move_lister(master_board, piece_coordinates, threat_map)
    blocking_moves = []
    ...
    for i in range(0,len(legal_moves)):
        ...
        hypo_threat_map = threat_mapper(hypo_master_board)
        check_flag = False
        for j in range(0,8):
            for k in range(0,8):
...
def threat_mapper(master_board):
    global clock, display_surface, square_size, database, piece_mapper
    global move_flags, turn
    ...
    for i in range(0,8):
        for j in range(0,8):
            if master_board[i][j] != 'NULL' and turn not in master_board[i][j]:
                legal_moves = threat_lister(master_board, [i,j])
